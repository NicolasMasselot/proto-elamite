...
## 2026-07-27 — Raw subset closure under the two existing controls

- Attempted: read the upper-tail p-value for the **raw** subset-sum closure
  statistic (observed 151/425) under the obverse-only null and the
  position-plus-magnitude null. No new nulls were defined, no new seed was
  introduced, and no new sampling was performed.
- **These numbers come from the existing fixed-seed draw sets.** Both the
  Session 6 obverse-only draws and the Session 6 position-plus-magnitude
  draws were regenerated by deterministic replay of seed `20260727` for the
  same 5000 draws, with the same two already-specified randomizers
  (`randomize_obverse_bundles_by_slot_count` over the 425 encoded candidates,
  and `randomize_tablet_notation_strata` over surface-by-magnitude pools).
  The scoring function never touches the RNG, so scoring paired raw and
  witness counts per draw leaves the pseudo-random sequence bit-identical to
  the run that produced the recorded witness results. This is the same
  extraction procedure already used for the full-sum raw p-values in the
  final definitional check.
- Reproduction check, run before reading any raw upper tail: the replay
  returns the previously recorded null means exactly — **63.3836** under the
  obverse-only null and **117.5020** under the position-plus-magnitude null.
  The witness margins of the same draw sets also reproduce exactly (obverse
  only: mean 12.3854, std 2.7201, range 4--22; position plus magnitude: mean
  21.7976, std 3.2497, range 11--35), confirming these are the same draw sets
  and not a fresh sample.
- Obverse-only null, raw subset closure: observed 151; null mean 63.3836;
  null std 5.1710; null range 46--86; 0 of 5000 draws reached 151;
  `p = 1/5001 = 0.000200`.
- Position-plus-magnitude null, raw subset closure: observed 151; null mean
  117.5020; null std 6.4662; null range 94--138; 0 of 5000 draws reached 151;
  `p = 1/5001 = 0.000200`.
- **This does not overturn the Session 4b negative result, and must not be
  reported as if it did.** The chance-level finding for raw subset closure
  (151 observed versus null mean 147.6738, `p = 0.370526`) is a statement
  about the *unstratified pool-wide* null, in which every notation slot in
  the corpus is exchangeable. The two nulls replayed here are sharper and
  hold different things fixed: the obverse-only null freezes every reverse
  summary and permutes whole obverse bundles only among tablets with equal
  obverse slot counts, and the position-plus-magnitude null resamples within
  surface-by-magnitude strata. Raw subset closure exceeds both of these while
  sitting at chance against the pool-wide null. The honest reading is that
  the pool-wide null is the permissive one that the permissive closure
  definition survives, not that subset closure is now an above-chance result
  in general. Report all three side by side; do not select the null that
  produces the lowest p-value.
- Ruled out: adding a null, changing a seed, changing the draw count,
  changing the closure definitions, or revising the Session 6 decision rule
  (which had already closed further testing for the witness result).
- Next session first: whenever the subset raw closure result is cited, name
  the null it was computed against in the same sentence. "Raw subset closure
  is at chance" is only true of the pool-wide null.
